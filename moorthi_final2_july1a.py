...
#moorthi_final2_july1.py     Modified for the new eth5 column in vm sheet and the
#                               different column names and order in network sheet

#moorthi_final2_july1a.py    Added checks in the main section to compare the column names
#                               listed by the script against the column names in the sheet
...
def read_server_defn(fname,sname):
    ...
    df_colnames_check_df=pd.read_excel(fname,sheet_name=sname,skiprows=1,nrows=0)
    df_colnames_check_columns=list(df_colnames_check_df.columns)

    if df_colnames_check_columns != server_sheet_columns:
        print("Column names in the sheet do not match the expected columns list")
        print(df_colnames_check_columns)
        print(server_sheet_columns)
        exit()



    df = pd.read_excel (fname, sheet_name=sname,skiprows=1,names=server_sheet_columns)
...
def read_server_network_defn(fname,sname):
        ...
        df_colnames_check_df=pd.read_excel(fname,sheet_name=sname,skiprows=9,nrows=0)
        df_colnames_check_columns=list(df_colnames_check_df.columns)

        if df_colnames_check_columns != network_sheet_columns:
            print("Column names in the sheet do not match the expected columns list")
            print(df_colnames_check_columns)
            print(network_sheet_columns)
            exit()
